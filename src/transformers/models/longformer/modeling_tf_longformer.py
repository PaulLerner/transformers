...
class TFLongformerSelfAttention(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        diagonal_mask = self._sliding_chunks_query_key_matmul(
            tf.ones(shape_list(attention_mask)), attention_mask, self.one_sided_attn_window_size
        )
        ...
        masked_global_attn_index = tf.cond(
            ...
            lambda: tf.tile(
                is_index_global_attn[:, :, None, None], (1, 1, self.num_heads, self.one_sided_attn_window_size * 2 + 1)
            ),
        )
        attn_probs = tf.where(
        ...
        **kwargs,
        ...
        if inputs["global_attention_mask"] is None and inputs["input_ids"] is not None:
            ...
            inputs["global_attention_mask"] = tf.tensor_scatter_nd_update(
                inputs["global_attention_mask"], indices, updates
            )
